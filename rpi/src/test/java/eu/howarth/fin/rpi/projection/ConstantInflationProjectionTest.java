...
class ConstantInflationProjectionTest {

    private static final RpiEntry ANCHOR = new RpiEntry(2024, 1, bd("100"));

    @Test
    void upToEqualsAnchor_returnsEmpty() {
        var projection = new ConstantInflationProjection(BigDecimal.ZERO);
    ...
    void zeroRate_correctMonthCount() {
        var projection = new ConstantInflationProjection(BigDecimal.ZERO);
        List<RpiEntry> entries = projection.project(ANCHOR, YearMonth.of(2024, 3));
    ...
    void crossesYearBoundary_correctMonthCount() {
        var projection = new ConstantInflationProjection(BigDecimal.ZERO);
        List<RpiEntry> entries = projection.project(ANCHOR, YearMonth.of(2025, 1));
    ...
    void positiveRate_firstMonthIndexExceedsAnchor() {
        var projection = new ConstantInflationProjection(bd("0.12"));
        List<RpiEntry> entries = projection.project(ANCHOR, YearMonth.of(2024, 2));
    ...
    void positiveRate_indexGrowsMonotonically() {
        var projection = new ConstantInflationProjection(bd("0.03"));
        List<RpiEntry> entries = projection.project(ANCHOR, YearMonth.of(2025, 1));
